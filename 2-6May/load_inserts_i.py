...
def createTempTable(conn):
	...
	with conn.cursor() as cursor:
		...
		print(f"Created unlogged staging table")
...
def load(conn, rlis):
        ...
        with conn.cursor() as cursor:
                ...
                #for cmd in icmdlist:
                #	cursor.execute(cmd)
                #cursor.execute(f"INSERT INTO {TableName} select * from staging")

                fileObject = io.StringIO()

                for row in rlis:
                        hispanic = float(row['Hispanic']) if row['Hispanic'] else 0
                        fileObject.write(','.join((
				row['CensusTract'],
				row['State'],
				row['County'],
				row['TotalPop'],
                ...
                cursor.copy_from(fileObject, TableName, sep=",")
